...
class Chatbot:
    def __init__(self, model_name="BAAI/bge-large-en-v1.5", max_chunk_size=250):
    ...
    def query(self, user_query):
        ...
        similar_chunks = self.chunker.search_similar_chunks(query_embedding, top_k=5)

        # Concatenate chunks into a single string
        result_string = "\n\n".join([chunk for chunk, _ in similar_chunks])

        # Use LLM to generate response
        system_part = f"You are an expert in Question and Answering. Answer the question using the text below. If the answer isn't present, respond with 'I am sorry. I don't know the answer.'\n{result_string}"
        prompt = (
